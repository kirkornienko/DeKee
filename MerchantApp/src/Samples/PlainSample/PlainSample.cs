...
using static PlainSample.Extensions;
    ...
    public class PlainSample
     {
        public void Execute()
        {
            ...
            R(()=> IndexTweet(tweet, "1", connection, serializer));

            R(() => IndexTweet(anotherTweet, "2", connection, serializer));

            R(() => BulkTweetIndex(tweets, connection, serializer));

            R(() => CreateIndexAlias(connection, serializer));

            //R(()=> ListIndexAliases(connection, serializer));

            R(() => GetTweet("1", serializer, connection));

            R(() => SearchTweets(connection, serializer));

            //R(()=> SearchTweetsAsync(connection, serializer).Wait());

            R(() => CountTweets(connection, serializer));

            R(() => DeleteTweeterIndex(connection, serializer));
